...
if( argc == 2 ) {
	...
	char* enterWord = (char*) malloc(sizeofWord * sizeof(char));
	enterWord=argv[1];
	anagram(enterWord);
	}
else if( argc > 2 ) {
 ...
 //printing on the screen after taking the user input
   char* enterWord = (char*) malloc(sizeofWord * sizeof(char));
   printf("Enter a word for the anagram program\n");
	scanf("%s",enterWord);
	anagram(enterWord);
	}
  freeMemory();
